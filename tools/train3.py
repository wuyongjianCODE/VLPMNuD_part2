...
def prepare_for_CCRCC_GT_detection(phase):#wuyongjian: used for OUR ccrcc!!!!!!!!!!!train3.py edited for this
    SWITCH={'Valid':'val',
            'Train':'train',
            'Test':'val',
            }
    # assert isinstance(dataset, COCODataset)
    if not os.path.exists('/data1/wyj/M/datasets/COCO_ccrcc/'):
        os.mkdir('/data1/wyj/M/datasets/COCO_ccrcc/')
    if not os.path.exists('/data1/wyj/M/datasets/COCO_ccrcc/{}2017/'.format(SWITCH[phase])):
        os.mkdir('/data1/wyj/M/datasets/COCO_ccrcc/{}2017/'.format(SWITCH[phase]))
    if not os.path.exists('/data1/wyj/M/datasets/COCO_ccrcc/annotations/'):
        os.mkdir('/data1/wyj/M/datasets/COCO_ccrcc/annotations/')
    ...
    k=0
    ALLmasks_dir = '/data1/wyj/M/datasets/ccrcccrop/{}/Instance_mask'.format(phase)
    for id,image_name in enumerate(os.listdir(ALLmasks_dir)):
        ...
        shutil.copyfile('/data1/wyj/M/datasets/ccrcccrop/{}/Images/{}'.format(phase,original_id),'/data1/wyj/M/datasets/COCO_ccrcc/{}2017/{}'.format(SWITCH[phase],original_id))
        coco.dataset["images"].append({"id": id,
                                       "height": 256, "width": 256, "file_name":original_id})
        ...
        # plt.show()
        # pass
    with open('/data1/wyj/M/datasets/COCO_ccrcc/annotations/instances_{}2017.json'.format(SWITCH[phase]), "w") as f:
...
if __name__ == "__main__":
    ...
    # prepare_for_CCRCC_GT_detection('Train')
    # prepare_for_CCRCC_GT_detection('Test')
    import os
    os.system('rm datasets/COCO')
    os.system('ln -s /data1/wyj/M/datasets/COCO_ccrcc datasets/COCO')
