...
def prepare_for_CONSEP_GT_detection(phase):#wuyongjian: used for OUR CONSEP!!!!!!!!!!!train2.py edited for this
    # assert isinstance(dataset, COCODataset)
    SWITCH={'Valid':'val',
            'Train':'train',
            'Test':'val',
            }
    if not os.path.exists('/data1/wyj/M/datasets/COCO_consep/'):
        os.mkdir('/data1/wyj/M/datasets/COCO_consep/')
    if not os.path.exists('/data1/wyj/M/datasets/COCO_consep/{}2017/'.format(SWITCH[phase])):
        os.mkdir('/data1/wyj/M/datasets/COCO_consep/{}2017/'.format(SWITCH[phase]))
    if not os.path.exists('/data1/wyj/M/datasets/COCO_consep/annotations/'):
        os.mkdir('/data1/wyj/M/datasets/COCO_consep/annotations/')
    ...
    k=0
    ALLmasks_dir = '/data1/wyj/M/datasets/consepcrop/{}/Instance_mask'.format(phase)
    for id,image_name in enumerate(os.listdir(ALLmasks_dir)):
        crop_size=250
        original_id = image_name+'.png'
        # IMO=io.imread('/data1/wyj/M/datasets/MoNuSACCROP/images/'+original_id)
        # plt.imshow(IMO)
        masks_dir=ALLmasks_dir+'/'+image_name
        print("image_id:{}---filename:{}".format(id, original_id))
        shutil.copyfile('/data1/wyj/M/datasets/consepcrop/{}/Images/{}'.format(phase,original_id),'/data1/wyj/M/datasets/COCO_consep/{}2017/{}'.format(SWITCH[phase],original_id))
        coco.dataset["images"].append({"id": id,
                                       "height": 250, "width": 250, "file_name":original_id})
        for instance_mask_file in os.listdir(masks_dir):
            instance_im=io.imread(masks_dir+'/'+instance_mask_file)
            connection_map=measure.label(instance_im)
        ...
        # plt.show()
        # pass
    with open('/data1/wyj/M/datasets/COCO_consep/annotations/instances_{}2017.json'.format(SWITCH[phase]), "w") as f:
...
if __name__ == "__main__":
    ...
    # preprocess_glip_result('TRAIN2.json',visual=True)
    # os.system('mv YOLOX_outputs/yolox_s YOLOX_outputs/yolox_s_many4n')
    # prepare_for_CONSEP_GT_detection('Train')
    # prepare_for_CONSEP_GT_detection('Test')
    import os
    os.system('rm datasets/COCO')
    os.system('ln -s /data1/wyj/M/datasets/COCO_consep datasets/COCO')
